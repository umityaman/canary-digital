...
export default function EInvoiceList() {
  ...
  const buildHeaders = (withJson = false) => {
    ...
    if (!token) {
      toast.error('Oturum bilgisi bulunamadý')
      return null
    }
  ...
  const handleSendEDocument = async (invoice: EInvoice) => {
    if (invoice.invoiceType === 'e-fatura') {
      ...
      try {
        ...
        if (!generateResponse.ok) {
          throw new Error(generatePayload?.message || 'E-Fatura XML oluþturulamadý')
        }
        ...
        toast.success('E-Fatura GÝB\'e gönderildi')
        loadInvoices()
      } catch (error: any) {
    ...
    if (!invoice.parasutId) {
      toast.error('Paraþüt fatura ID bulunamadý')
      return
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(payload?.message || 'E-Arþiv gönderilemedi')
      }

      toast.success('E-Arþiv baþarýyla oluþturuldu')
      loadInvoices()
    } catch (error: any) {
      console.error('Failed to send e-archive:', error)
      toast.error(error.message || 'E-Arþiv gönderilemedi')
    } finally {
      setActionInvoiceId(null)
    }
  }
  ...
  const handleCheckEInvoiceStatus = async (invoice: EInvoice) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(payload?.message || 'Durum sorgulanamadý')
      }

      toast.success(`GÝB durumu: ${payload?.data?.status || 'bilinmiyor'}`)
      loadInvoices()
    } catch (error: any) {
      console.error('Failed to check e-invoice status:', error)
      toast.error(error.message || 'Durum sorgulanamadý')
    } finally {
      setActionInvoiceId(null)
    }
  }
  ...
  const handleCancelInvoice = async (invoice: EInvoice) => {
    if (!confirm(`${invoice.invoiceNumber} numaralý faturayý iptal etmek istediðinize emin misiniz?`)) {
      return
    }
  ...
  const getStatusBadge = (status: string) => {
    const badges = {
      ...
      sent: { label: 'Gönderildi', color: 'bg-neutral-100 text-neutral-800', icon: <Send size={14} /> },
      approved: { label: 'Onaylandý', color: 'bg-neutral-100 text-neutral-800', icon: <CheckCircle size={14} /> },
      rejected: { label: 'Reddedildi', color: 'bg-neutral-100 text-neutral-800', icon: <X size={14} /> },
      cancelled: { label: 'Ýptal', color: 'bg-neutral-100 text-neutral-800', icon: <AlertCircle size={14} /> },
      paid: { label: 'Ödendi', color: 'bg-neutral-100 text-neutral-800', icon: <CheckCircle size={14} /> },
    }
  ...
  const getEDocumentStatusBadge = (invoice: EInvoice) => {
    if (invoice.invoiceType === 'e-fatura') {
      ...
      const badges = {
        ...
        sent: { label: 'GÝB Gönderildi', color: 'bg-neutral-100 text-neutral-800', icon: <Send size={12} /> },
        delivered: { label: 'Yanýt Alýndý', color: 'bg-neutral-100 text-neutral-800', icon: <CheckCircle size={12} /> },
    ...
    const badges = {
      ...
      sent: { label: 'Gönderildi', color: 'bg-neutral-100 text-neutral-800', icon: <Send size={12} /> },
      delivered: { label: 'Teslim Edildi', color: 'bg-neutral-100 text-neutral-800', icon: <CheckCircle size={12} /> },
      failed: { label: 'Baþarýsýz', color: 'bg-neutral-100 text-neutral-800', icon: <AlertCircle size={12} /> },
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className={`${DESIGN_TOKENS?.typography?.h2} ${DESIGN_TOKENS?.colors?.text.primary}`}>E-Fatura & E-Arþiv</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
        ...
        <div className={cx(card('md', 'sm', 'subtle', 'lg'), 'bg-gradient-to-br from-indigo-50 to-indigo-100 border-indigo-200')}>
          ...
          <p className="text-xs text-neutral-800">E-Arþiv</p>
        </div>
        ...
        <div className={cx(card('md', 'sm', 'subtle', 'lg'), 'bg-gradient-to-br from-orange-50 to-orange-100 border-orange-200')}>
          ...
          <p className="text-xs text-neutral-800">Ödenmemiþ</p>
        </div>
      </div>
        ...
        <div className="flex flex-col lg:flex-row gap-4">
          ...
          <div className="flex-1">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Fatura no, müþteri, vergi no ara..."
                value={searchTerm}
          ...
          >
            ...
            <option value="e-arsiv">E-Arþiv</option>
          </select>
          ...
          <div className="p-12 text-center">
            ...
            <p className="text-neutral-600">
              {searchTerm || filterType !== 'all' || filterStatus !== 'all'
                ? 'Arama kriterlerine uygun fatura bulunamadý'
                  ...
                  >
                      ...
                      <div>
                        ...
                        {invoice.parasutInvoiceNo && (
                          <div className="text-xs text-neutral-900 mt-1 flex items-center gap-1">
                            ...
                            Paraþüt: {invoice.parasutInvoiceNo}
                          </div>
                        )}
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4 text-center">
                      <span className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-medium ${
                        ...
                        {invoice.invoiceType === 'e-fatura' ? 'E-Fatura' : 'E-Arþiv'}
                      </span>
                    </td>
