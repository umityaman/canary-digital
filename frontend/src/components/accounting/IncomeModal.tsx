...
import { Income, INCOME_CATEGORIES, INCOME_STATUS, PAYMENT_METHODS } from './IncomeTab'
...
export default function IncomeModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingIncome) {
        await accountingAPI.updateIncome(editingIncome.id, data)
        toast.success('Gelir kaydı güncellendi')
      } else {
        await accountingAPI.createIncome(data)
      ...
      <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b border-neutral-200">
          <h2 className="text-xl font-semibold text-neutral-900">
            {editingIncome ? 'Gelir Kaydını Düzenle' : 'Yeni Gelir Kaydı'}
          </h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder="Örn: Ekipman Kiralama Geliri"
              required
            />
          </div>
