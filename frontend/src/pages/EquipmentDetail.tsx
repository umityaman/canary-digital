...
const EquipmentDetail: React.FC = () => {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'RENTED':
        return 'bg-blue-100 text-blue-800 border-neutral-200';
      case 'RESERVED':
      ...
      case 'LOST':
        return 'bg-neutral-100 text-gray-800 border-neutral-200';
      default:
        return 'bg-neutral-100 text-gray-800 border-neutral-200';
    }
  };
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-neutral-200 p-6">
        <div className="flex items-center mb-4">
          ...
          <h2 className="text-lg font-semibold text-neutral-900">Fiyatlandırma</h2>
        </div>
        <div className="space-y-3">
          {equipment?.hourlyPrice && (
            <div className="flex justify-between items-center">
              <span className="text-neutral-600">Saatlik</span>
              <span className="font-semibold text-neutral-900">{equipment.hourlyPrice.toLocaleString('tr-TR')} ₺</span>
            </div>
          )}
          {equipment?.dailyPrice && (
            <div className="flex justify-between items-center">
              <span className="text-neutral-600">Günlük</span>
              <span className="font-semibold text-neutral-900">{equipment.dailyPrice.toLocaleString('tr-TR')} ₺</span>
            </div>
          )}
          {equipment?.weeklyPrice && (
            <div className="flex justify-between items-center">
              <span className="text-neutral-600">Haftalık</span>
              <span className="font-semibold text-neutral-900">{equipment.weeklyPrice.toLocaleString('tr-TR')} ₺</span>
            </div>
          )}
          {equipment?.monthlyPrice && (
            <div className="flex justify-between items-center">
              <span className="text-neutral-600">Aylık</span>
              <span className="font-semibold text-neutral-900">{equipment.monthlyPrice.toLocaleString('tr-TR')} ₺</span>
            </div>
          )}
          {equipment?.weekendPrice && (
            <div className="flex justify-between items-center">
              <span className="text-neutral-600">Hafta Sonu</span>
              <span className="font-semibold text-neutral-900">{equipment.weekendPrice.toLocaleString('tr-TR')} ₺</span>
            </div>
          )}
      ...
      <div className="bg-white rounded-xl shadow-sm border border-neutral-200 p-6">
        <div className="flex items-center mb-4">
          ...
          <h2 className="text-lg font-semibold text-neutral-900">Mali Bilgiler</h2>
        </div>
        <div className="space-y-3">
          {equipment?.replacementValue && (
            <div className="flex justify-between items-center">
              <span className="text-neutral-600">Yenileme Bedeli</span>
              <span className="font-semibold text-neutral-900">{equipment.replacementValue.toLocaleString('tr-TR')} ₺</span>
            </div>
          )}
          {equipment?.depositAmount && (
            <div className="flex justify-between items-center">
              <span className="text-neutral-600">Depozito</span>
              <span className="font-semibold text-neutral-900">{equipment.depositAmount.toLocaleString('tr-TR')} ₺</span>
            </div>
          )}
      ...
      <div className="bg-white rounded-xl shadow-sm border border-neutral-200 p-6">
        <div className="flex items-center mb-4">
          ...
          <h2 className="text-lg font-semibold text-neutral-900">Kiralama Bilgileri</h2>
        </div>
        <div className="space-y-3">
          {equipment?.minRentalPeriod && (
            <div className="flex justify-between items-center">
              <span className="text-neutral-600">Min. Süre</span>
              <span className="font-semibold text-neutral-900">{equipment.minRentalPeriod} gün</span>
            </div>
          )}
          {equipment?.maxRentalPeriod && (
            <div className="flex justify-between items-center">
              <span className="text-neutral-600">Max. Süre</span>
              <span className="font-semibold text-neutral-900">{equipment.maxRentalPeriod} gün</span>
            </div>
          )}
          ...
          {equipment?.orderItems && equipment.orderItems.length > 0 ? (
            <div className="space-y-4">
              {equipment.orderItems.map((item) => (
                <div key={item.id} className="border border-neutral-200 rounded-lg p-4 hover:shadow-md transition">
                  <div className="flex justify-between items-start mb-2">
                    <div>
                      <h4 className="font-semibold text-neutral-900">Sipariş #{item.order.orderNumber}</h4>
                      <p className="text-sm text-neutral-600 mt-1">{item.order.customer.name}</p>
          ...
          {equipment?.inspections && equipment.inspections.length > 0 ? (
            <div className="space-y-4">
              {equipment.inspections.map((inspection) => (
                <div key={inspection.id} className="border border-neutral-200 rounded-lg p-4 hover:shadow-md transition">
                  <div className="flex justify-between items-start mb-2">
                    <div>
                      ...
                      <p className="text-sm text-neutral-600 mt-1">Yetkili: {inspection.inspector.name}</p>
  ...
  const renderCalendarTab = () => (
    <div className="bg-white rounded-xl shadow-sm border border-neutral-200 p-6">
      <div className="text-center py-12">
        ...
        <h3 className="text-lg font-semibold text-neutral-900 mb-2">Müsaitlik Takvimi</h3>
  ...
  return (
    <div className="max-w-7xl mx-auto p-6 space-y-6">
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="flex items-center px-4 py-2 bg-neutral-100 text-neutral-700 rounded-lg hover:bg-neutral-200 transition"
              >
          ...
          <div className="flex items-start gap-6">
            ...
            <div className="flex-shrink-0">
              {equipment.imageUrl ? (
                ...
                <div className="w-32 h-32 bg-neutral-100 rounded-lg border border-neutral-200 flex items-center justify-center">
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center gap-6 text-sm text-gray-500 mb-4">
                <span>Kod: <strong className="text-neutral-900">{equipment.code}</strong></span>
                {equipment.serialNumber && (
                  <span>Seri No: <strong className="text-neutral-900">{equipment.serialNumber}</strong></span>
                )}
                <span>Adet: <strong className="text-neutral-900">{equipment.quantity}</strong></span>
              </div>

              {equipment.description && (
                <p className="text-neutral-700 leading-relaxed">{equipment.description}</p>
              )}
            </div>
          </div>
        </div>
              ...
              return (
                <button
                  ...
                  className={`py-4 px-1 border-b-2 font-medium text-sm flex items-center transition ${
                    activeTab === tab.key
                      ? 'border-neutral-500 text-blue-600'
      ...
      <div className="bg-white rounded-xl shadow-sm border border-neutral-200 p-6">
        <h2 className="text-lg font-semibold text-neutral-900 mb-4">Ek Bilgiler</h2>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          <div>
            <span className="text-neutral-600">Oluşturulma</span>
            <p className="font-medium text-neutral-900 mt-1">
              {new Date(equipment.createdAt).toLocaleDateString('tr-TR')}
            </p>
          </div>
          <div>
            <span className="text-neutral-600">Son Güncelleme</span>
            <p className="font-medium text-neutral-900 mt-1">
          ...
          {equipment.qrCode && (
            <div>
              <span className="text-neutral-600">QR Kod</span>
              <p className="font-medium text-neutral-900 mt-1">{equipment.qrCode}</p>
            </div>
          )}
          {equipment.barcode && (
            <div>
              <span className="text-neutral-600">Barkod</span>
              <p className="font-medium text-neutral-900 mt-1">{equipment.barcode}</p>
            </div>
          )}
        </div>
      </div>
